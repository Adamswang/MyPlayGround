    ...
    if df.empty is False:
        ...
        for i, item in enumerate(df.columns): # 预留的枚举，这个方法以后可以根据输入对象不同从单一柱状图变成堆积柱状图
            stackbar.add_yaxis(item,
                      ...
                      itemstyle_opts=opts.ItemStyleOpts(color='#6A93B0')
                      )
        ...
        stackbar.set_global_opts(
            ...
            xaxis_opts=opts.AxisOpts(type_='category',
                                     boundary_gap=True,
                                     axislabel_opts=opts.LabelOpts(rotate=60),  # x轴标签方向rotate有时能解决拥挤显示不全的问题
        ...
        if df_gr is not None:
            line = (
                Line().add_xaxis(xaxis_data=df_gr.index.tolist()).add_yaxis(
                    series_name="同比增长率",
                    yaxis_index=1,
